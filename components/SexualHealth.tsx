...
const SexualHealth: React.FC = () => {
  ...
  const [activePrompt, setActivePrompt] = useState("What was the most challenging part of your shift today?");
  ...
  const sideEffects = [
    ...
    { id: 'mood', label: 'Mood', guidance: 'Hormones can impact neurotransmitters.', tip: 'Log mood daily to find patterns.', alert: 'Thoughts of self-harm or depression.' },
    { id: 'headache', label: 'Headaches', guidance: 'Can happen during hormone-free weeks.', tip: 'Stay hydrated and sleep well.', alert: 'Migraine with aura (visual flashes) - Stop pill immediately.' },
    { id: 'libido', label: 'Low Libido', guidance: 'Some methods lower testosterone slightly.', tip: 'Discuss switching methods.', alert: 'Persistent relationship distress.' },
  ];
  ...
  const shufflePrompt = () => {
    const prompts = ["What went well today?", "How does your body feel?", "What are you looking forward to?", "Did you feel safe today?"];
          ...
          {mentalTab === 'DASHBOARD' && !activeAssessment && (
            <div className="space-y-6 animate-slide-up">
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-800 dark:to-black rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-xl">
                ...
                <h2 className="text-2xl font-bold mb-2 relative z-10">Good Afternoon</h2>
                <p className="text-slate-300 text-sm mb-8 max-w-[80%] relative z-10">Taking a moment to check in with yourself can prevent burnout later.</p>
                <button onClick={() => setIsCheckInOpen(true)} className="w-full py-4 bg-white text-slate-900 rounded-2xl font-bold text-sm shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2 relative z-10 hover:bg-slate-50"><Icons.Leaf className="w-5 h-5 text-teal-600" />Start Daily Check-in</button>
              </div>
              <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 shadow-sm border border-slate-100 dark:border-slate-800">
                ...
                <div className="flex justify-between mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-bold text-slate-900 dark:text-white">Thursday</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-slate-500 dark:text-slate-400">Streak</p>
                    <p className="text-sm font-bold text-teal-600 dark:text-teal-400">5 days</p>
                  </div>
                  <div className="text-center">
        ...
        <div className="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between mt-6">
              <button disabled={examStep === 0} onClick={() => setExamStep(s => s - 1)} className="px-6 py-3 rounded-xl text-xs font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">Previous</button>
              <button disabled={examStep === 2} onClick={() => setExamStep(s => s + 1)} className="px-6 py-3 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition-colors shadow-lg shadow-slate-200">Next Step</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="pt-14 px-6 pb-6 bg-slate-50/95 dark:bg-slate-950/95 sticky top-0 z-20 shadow-sm flex items-center gap-2 transition-colors duration-300 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-none">
            ...
            {view === 'MENTAL_HEALTH_VIEW' && 'Mental Health'}
          </h1>
        </div>
      </div>
